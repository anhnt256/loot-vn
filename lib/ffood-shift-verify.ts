...
export function getShiftApiDateParam(reportDate: string): string {
  ...
  return d.hour(17).minute(0).second(0).millisecond(0).toISOString().replace(/\.\d{3}Z$/, ".000Z");
}
...
function sumFromPayments(
  ...
  return payments.filter(filter).reduce((sum, p) => sum + (Number(p.total) || 0), 0);
}
 ...
 */
export function computePaymentTotals(payments: FfoodShiftPayment[] | undefined): {
  totalFood: number;
  deduction: number;
  actualFfood: number;
} {
 ...
 */
export async function fetchFfoodShifts(branch: string, reportDate: string): Promise<{
  success: boolean;
  error?: string;
  data?: FfoodShiftItem[];
  apiDateParam?: string;
  ...
  if (!token) {
    return { success: false, error: "Ffood token not set for branch (login or refresh token first)" };
  }
  if (!shopId) {
    return { success: false, error: "Ffood shop_id not set in FfoodCredential" };
  }
  ...
  const json = (await res.json()) as { data?: FfoodShiftItem[]; isSuccess?: boolean; errors?: unknown };
 ...
 */
export async function verifyFfoodShifts(branch: string, reportDate: string): Promise<FfoodShiftVerifyResult> {
  ...
  for (const ws of workShiftRows) {
    if (ws.ffood_id) workShiftsByFfoodId.set(ws.ffood_id.trim(), { id: ws.id, name: ws.name });
  }
  ...
  if (!fetchResult.success) {
    return {
      ...
      workShifts: workShiftRows.map((r) => ({ id: r.id, name: r.name, ffood_id: r.ffood_id })),
    };
  }
  ...
  for (const item of rawShifts) {
    ...
    const { totalFood, deduction, actualFfood } = computePaymentTotals(item.payments);
    verified.push({
      ffoodShiftId: item.id,
      workShiftId: ws.id,
      workShiftName: ws.name,
      employeeId,
      employeeName: [item.employee?.firstname, item.employee?.lastname].filter(Boolean).join(" ").trim() || undefined,
