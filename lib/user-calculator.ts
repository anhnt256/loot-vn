...
export async function calculateActiveUsersInfo(
  ...
  const [
    ...
    (async () => {
      ...
      const queryString = `
          SELECT systemlogtb.UserId, systemlogtb.EnterDate, systemlogtb.EndDate, systemlogtb.EnterTime, systemlogtb.EndTime, systemlogtb.status, u.UserType
          FROM fnet.systemlogtb
          INNER JOIN usertb u ON systemlogtb.UserId = u.UserId
          WHERE systemlogtb.UserId IN (${userIdsStr})
            AND systemlogtb.status = 3
            AND (
