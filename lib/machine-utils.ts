 ...
 */
export async function fetchMachineDetailsByGroup(machineGroupId?: number): Promise<MachineDetail[]> {
  try {
    ...
    if (!result.success) {
      throw new Error(result.error || 'Failed to fetch machine details by group');
    }
 ...
 */
export function getMachineStatus(netInfo: NetInfo): 'online' | 'offline' | 'busy' | 'idle' {
