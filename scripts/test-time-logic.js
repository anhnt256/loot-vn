...
const calculateDailyUsageTime = (sessions) => {
  ...
  console.log(
    "Yesterday start (VN):",
    yesterdayStart.format("YYYY-MM-DD HH:mm:ss"),
  );
  ...
  for (const session of sessions) {
    try {
      ...
      if (!session.EnterDate || !session.EnterTime) {
        console.log(
          "❌ Invalid session data - missing EnterDate/EnterTime:",
          session,
        );
        continue;
      }
      ...
      const enterDateTime = parseSessionDateTime(
        session.EnterDate,
        session.EnterTime,
      );
      ...
      if (enterDateStr === todayStr) {
        ...
        if (session.TimeUsed && session.TimeUsed > 0) {
          ...
          console.log(
            `✅ Today session with TimeUsed: ${session.TimeUsed} minutes`,
          );
          ...
          console.log(
            `🔄 Ongoing session, calculated: ${minutesPlayed} minutes`,
          );
        ...
        console.log(
          `🌙 Overnight session, adding from today start: ${minutesPlayedToday} minutes`,
        );
        totalMinutes += minutesPlayedToday;
      } else {
        console.log(
          `❌ Session not counted: ${enterDateStr} vs ${todayStr}/${yesterdayStr}`,
        );
      }
    } catch (error) {
...
console.log(
  "VN time:",
  dayjs(testTime).tz("Asia/Ho_Chi_Minh").format("YYYY-MM-DD HH:mm:ss"),
);
