import { db, GameAppointmentTier } from "@gateway-workspace/database";
...
export async function calculateTier(
  ...
  try {
    ...
    const eligibleTier = (tiers as GameAppointmentTier[]).find((tier: GameAppointmentTier) => {
...
export async function getTierDowngradeOptions(
  ...
  try {
    ...
    const eligibleTiers = (tiers as GameAppointmentTier[]).filter((tier: GameAppointmentTier) => {
      const meetsMinMembers = members >= tier.minMembers;
      const meetsMaxMembers = !tier.maxMembers || members <= tier.maxMembers;
      const meetsMinHours = hours >= tier.minHours;
    ...
    return eligibleTiers.map((tier: GameAppointmentTier) => {
...
export async function getTierStatistics(): Promise<{
  ...
  try {
    ...
    const tierDistribution = (tiers as GameAppointmentTier[]).map((tier: GameAppointmentTier) => {
      const tasks = tier.tasks as any;
    ...
    return {
      ...
      activeTiers: (tiers as GameAppointmentTier[]).filter((tier: GameAppointmentTier) => tier.isActive).length,
      tierDistribution: (tierDistribution as any[]).map((tier: any) => ({
        ...tier,
        maxMembers: tier.maxMembers === null ? undefined : tier.maxMembers,
      })),
    };
