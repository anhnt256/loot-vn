...
import { db, getFnetDB, MainPrisma } from "@gateway-workspace/database";
import { cookies } from "next/headers";
import { createSafeAction } from "../../create-safe-action";
import { checkGameRollRateLimit } from "../../rate-limit";

import { GameItemResults, InputType, ReturnType } from "./type";
import { CreateGameResult } from "./schema";
import { getCurrentTimeVNISO, getCurrentTimeVNDB, getStartOfWeekVNISO, getEndOfWeekVNISO } from "../../timezone-utils";
...
const handler = async (data: InputType): Promise<ReturnType> => {
  ...
  if (type === "Gift") {
    ...
    try {
      return await db.$transaction(async (tx: MainPrisma.TransactionClient) => {
