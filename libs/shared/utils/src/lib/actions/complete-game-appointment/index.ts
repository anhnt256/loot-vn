...
import { completeAppointment } from '../../reward-distribution';
import { notifyAppointmentCompleted } from '../../game-appointment-notifications';
...
export const completeGameAppointmentAction = createSafeAction(
  ...
  async (data): Promise<CompleteGameAppointmentResponse> => {
    try {
      ...
      const memberIds = appointment.members.map((m: any) => m.userId);
      const completedMemberIds = data.completedMembers.map(m => m.userId);
      
      const missingMembers = memberIds.filter((id: any) => !completedMemberIds.includes(id));
      ...
      const forfeitedAmount = data.completedMembers
        ...
        .reduce((sum, member) => {
          const memberData = appointment.members.find((m: any) => m.userId === member.userId);
