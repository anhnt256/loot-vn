...
// Bind x-tenant-id from hostname (e.g. hr-manager-gateway-go-vap.loot.vn -> gateway-go-vap) or env
if (typeof globalThis.window !== 'undefined') {
  const host = globalThis.window.location.hostname;
  const firstSegment = host.split('.')[0] ?? '';
  const slug = firstSegment.startsWith('hr-manager-')
    ? firstSegment.slice('hr-manager-'.length)
