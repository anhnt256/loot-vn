...
function parseTimeToHHmmss(input: string): string {
  ...
  return [Math.min(23, Math.max(0, h)), Math.min(59, Math.max(0, m)), Math.min(59, Math.max(0, s))].map((n) => n.toString().padStart(2, "0")).join(":");
}
...
export async function POST(request: NextRequest) {
  try {
    ...
    const { name, startTime, endTime, isOvernight, FnetStaffId, FfoodStaffId } = body;
    ...
    if (existing.length > 0) {
      return NextResponse.json({ success: false, error: `Ca ${nameStr} đã tồn tại cho chi nhánh` }, { status: 400 });
    }
    ...
    return NextResponse.json({
      success: true,
      data: { ...shift, startTime: formatTime(shift.startTime), endTime: formatTime(shift.endTime) },
    });
  } catch (error) {
    console.error("Error creating work shift:", error);
    return NextResponse.json({ success: false, error: "Failed to create work shift" }, { status: 500 });
  }
}
...
export async function PUT(request: NextRequest) {
  try {
    ...
    const body = await request.json();
    const { id, name, startTime, endTime, isOvernight, FnetStaffId, FfoodStaffId } = body;
    if (!id) {
      return NextResponse.json({ success: false, error: "id is required" }, { status: 400 });
    }
    const existing = (await db.$queryRawUnsafe("SELECT id, name FROM WorkShift WHERE id = ? AND branch = ?", Number(id), branch)) as any[];
    if (existing.length === 0) {
      return NextResponse.json({ success: false, error: "Work shift not found" }, { status: 404 });
    }
    if (name !== undefined) {
      const nameStr = typeof name === "string" ? name.trim() : "";
      if (!nameStr) return NextResponse.json({ success: false, error: "name cannot be empty" }, { status: 400 });
    }
    const nameStr = name !== undefined ? (typeof name === "string" ? name.trim() : "") : undefined;
    if (nameStr && nameStr !== existing[0].name) {
      const dup = (await db.$queryRawUnsafe("SELECT id FROM WorkShift WHERE branch = ? AND name = ? AND id != ?", branch, nameStr, Number(id))) as any[];
      if (dup.length > 0) {
        return NextResponse.json({ success: false, error: `Ca ${nameStr} đã tồn tại` }, { status: 400 });
      }
    }
    const updates: string[] = [];
    const values: any[] = [];
    if (nameStr !== undefined) { updates.push("name = ?"); values.push(nameStr); }
    if (startTime !== undefined) { updates.push("startTime = ?"); values.push(parseTimeToHHmmss(startTime)); }
    if (endTime !== undefined) { updates.push("endTime = ?"); values.push(parseTimeToHHmmss(endTime)); }
    if (isOvernight !== undefined) { updates.push("isOvernight = ?"); values.push(!!isOvernight); }
    if (FnetStaffId !== undefined) { updates.push("FnetStaffId = ?"); values.push(FnetStaffId == null ? null : Number(FnetStaffId)); }
    if (FfoodStaffId !== undefined) { updates.push("FfoodStaffId = ?"); values.push(FfoodStaffId == null ? null : Number(FfoodStaffId)); }
    if (updates.length === 0) {
    ...
    return NextResponse.json({ success: false, error: "Failed to update work shift" }, { status: 500 });
  }
}
...
export async function DELETE(request: NextRequest) {
  try {
    ...
    const existing = (await db.$queryRawUnsafe("SELECT id FROM WorkShift WHERE id = ? AND branch = ?", id, branch)) as any[];
    if (existing.length === 0) {
      return NextResponse.json({ success: false, error: "Work shift not found" }, { status: 404 });
    }
    await db.$executeRawUnsafe("DELETE FROM WorkShift WHERE id = ? AND branch = ?", id, branch);
    return NextResponse.json({ success: true, message: "Deleted" });
  } catch (error) {
    console.error("Error deleting work shift:", error);
    return NextResponse.json({ success: false, error: "Failed to delete work shift" }, { status: 500 });
  }
}
