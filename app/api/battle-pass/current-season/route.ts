...
export async function GET() {
  try {
    ...
    let currentSeason = await db.battlePassSeason.findFirst({
      where: {
        isActive: true,
        startDate: {
          lte: new Date(),
          ...
          rewards: {
            create: [
              ...
              ...Array.from({ length: 30 }, (_, i) => {
                ...
                return {
                  level,
                  name: isFree ? `Free Reward Lv.${level}` : `Premium Reward Lv.${level}`,
                  description: isFree ? `Free tier reward for level ${level}` : `Premium tier reward for level ${level}`,
              ...
              },
              {
                ...
                description: 'Premium ultimate reward for completing all levels',
                type: 'premium',
                rewardType: 'coins',
                rewardValue: 10000,
                isBonus: true,
