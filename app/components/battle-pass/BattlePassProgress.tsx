...
export function BattlePassProgress({ 
  ...
  const [selectedReward, setSelectedReward] = useState<BattlePassReward | null>(null);
  ...
  const isSeasonEnded = seasonEndDate ? new Date() >= new Date(seasonEndDate) : false;
  ...
  const RewardSlot = ({ reward, isPremiumSlot = false }: { reward?: BattlePassReward, isPremiumSlot?: boolean }) => {
    ...
    const canClaim = reward && currentXP >= reward.experience && !isClaimed && (!isPremiumSlot || isPremium) && !isSeasonEnded;

          return (
        <div className={`reward-slot relative w-full h-full rounded-lg border-2 ${
          isPremiumSlot 
            ? isLocked 
      ...
      onClick={() => reward && setSelectedReward(reward)}>
        ...
        {isLevelLocked && reward && (
          <>
            ...
            <div className="relative text-center">
              ...
              <div className="text-xs text-white font-bold">Lv.{reward.level}</div>
            </div>
          </>
        )}
        {reward && !isLocked && !isLevelLocked && (
          <div className="text-center">
            ...
            <div className="text-xs text-white font-bold">{reward.rewardValue?.toLocaleString()}</div>
          </div>
        )}
        {!reward && !isLocked && (
    ...
    return (
      <div className="flex flex-col items-center gap-1 min-w-[80px]">
        ...
        <div className="w-16 h-16">
          <RewardSlot reward={reward} isPremiumSlot={reward?.type === 'premium'} />
        </div>
  ...
  const sortedRewards = rewards.filter(r => !r.isBonus).sort((a, b) => a.level - b.level);
  let computedLevel = 0;
  let nextXP = 0;
    ...
    <div className="h-screen bg-gradient-to-b from-indigo-900 via-purple-900 to-pink-900 p-4 overflow-hidden">
      <div className="battle-pass-container h-full flex gap-6 lg:flex-row flex-col">
        ...
        <div className="flex items-center gap-4">
          ...
          <h1 className="text-2xl font-bold text-white">{seasonName || 'Hành Trình Chiến Đấu'}</h1>
        </div>
      ...
      <div className="flex items-center justify-between mb-8">
        <div className="flex items-center gap-6 text-white">
          ...
          <div className="flex items-center gap-6">
            ...
            {(() => {
              const claimableCount = rewards.filter(r => 
                !r.isBonus && 
              ...
              return claimableCount > 0 && !isSeasonEnded && (
                <div className="text-center">
                  <div className="text-sm opacity-80">Phần Thưởng Có Thể Nhận</div>
            ...
            <div className="w-48 h-4 bg-gray-700 rounded-full overflow-hidden">
              <div 
                ...
                style={{ width: `${nextXP > 0 ? Math.min((currentXP / nextXP) * 100, 100) : 0}%` }}
              />
            </div>
            <div className="text-sm">
              <div>{currentXP}/{nextXP} XP</div>
            </div>
          </div>
        </div>
        ...
        <div className="flex items-center gap-4">
          ...
          {(() => {
            ...
            return claimableRewards.length > 0 && !isSeasonEnded && (
              <Button 
                onClick={() => {
        ...
        {bonusRewards.length > 0 && (
          <div className="mt-8 p-6 bg-gradient-to-r from-purple-800/50 to-pink-800/50 rounded-lg border border-purple-500/30">
            <div className="flex items-center gap-2 text-white mb-4">
              ...
              <span className="text-sm opacity-80">(Hoàn thành tất cả 30 cấp)</span>
            </div>
            <div className="flex gap-6 justify-center">
              {bonusRewards.map((reward) => {
                const isClaimed = claimedRewards.includes(reward.id);
                const canClaim = currentLevel >= reward.level && !isClaimed && (reward.type === 'free' || isPremium) && !isSeasonEnded;
                ...
                return (
                  <div key={reward.id} className="text-center">
                    ...
                    <div className="text-white text-sm font-bold mt-2">{reward.name}</div>
                  </div>
                );
              })}
            </div>
      ...
      {/* Reward Detail Modal */}
      {selectedReward && (
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-4" onClick={e => { if (e.target === e.currentTarget) setSelectedReward(null); }}>
          <Card className="bg-gradient-to-b from-gray-900 to-gray-800 border-gray-600 max-w-lg w-full relative">
            <div className="p-6">
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h3 className="text-xl font-bold text-white">
                      {selectedReward.isBonus ? selectedReward.name : `Cấp ${selectedReward.level}`}
                    </h3>
                    {!selectedReward.isBonus && (
                      <p className="text-sm text-gray-400">Phần Thưởng Hành Trình</p>
                    )}
                  </div>
                </div>
              </div>
              ...
              <div className="text-center mb-6">
                <div className={`w-20 h-20 mx-auto mb-4 rounded-lg border-2 flex items-center justify-center ${
                  selectedReward.type === 'premium' 
                    ...
                    : selectedReward.isBonus
                      ...
                      : 'bg-gradient-to-b from-blue-400 to-cyan-500 border-blue-400'
                                  ...
                                  <div className="text-center">
                  <div className="text-2xl">{getRewardIcon(selectedReward.rewardType)}</div>
                  <div className="text-xs text-white font-bold">{selectedReward.rewardValue?.toLocaleString()}</div>
                </div>
                </div>
              ...
              <div className="space-y-3 mb-6">
                <div className="flex items-center justify-between p-3 bg-gray-800/50 rounded-lg">
                  <span className="text-gray-300">Giá Trị Phần Thưởng:</span>
                  <span className="text-yellow-400 font-bold">+{selectedReward.rewardValue?.toLocaleString()} {selectedReward.rewardType}</span>
                </div>
                ...
                <div className="flex items-center justify-between p-3 bg-gray-800/50 rounded-lg">
                  ...
                  <span className={`px-3 py-1 rounded-full text-sm font-bold ${
                    ...
                    {selectedReward.isBonus ? 'Bonus' : selectedReward.type === 'premium' ? 'Premium' : 'Miễn Phí'}
                  </span>
                </div>
                ...
                {!selectedReward.isBonus && (
                  <div className="flex items-center justify-between p-3 bg-gray-800/50 rounded-lg">
                    ...
                    <div className="text-right">
                      <div className="text-cyan-400 font-bold">{selectedReward.experience} EXP</div>
                    </div>
                  </div>
                )}

                {/* Progress to unlock */}
                {!selectedReward.isBonus && currentXP < selectedReward.experience && (
                  <div className="p-3 bg-orange-900/30 rounded-lg border border-orange-500/30">
                    <div className="flex items-center justify-between mb-2">
                      <span className="text-orange-300 text-sm">Tiến độ mở khóa:</span>
                      <span className="text-orange-400 font-bold">{Math.round((currentXP / selectedReward.experience) * 100)}%</span>
                    </div>
                    <div className="w-full h-2 bg-gray-700 rounded-full overflow-hidden">
                      <div 
                        className="h-full bg-gradient-to-r from-orange-400 to-orange-500 rounded-full transition-all duration-500"
                        style={{ width: `${Math.min((currentXP / selectedReward.experience) * 100, 100)}%` }}
                      />
                    </div>
                    <div className="text-xs text-gray-400 mt-1">
                      Cần thêm {selectedReward.experience - currentXP} EXP nữa
                    </div>
                  </div>
                )}
                ...
                {selectedReward.isBonus && (
                  <div className="p-3 bg-purple-900/30 rounded-lg border border-purple-500/30">
                    <div className="text-center">
                      <div className="text-purple-300 text-sm mb-1">Yêu Cầu Mở Khóa:</div>
                      <div className="text-purple-400 font-bold">Hoàn thành tất cả 30 cấp Hành Trình</div>
                      <div className="text-xs text-gray-400 mt-1">
                        {currentLevel >= 30 ? '✅ Đã đáp ứng yêu cầu!' : `Còn lại ${30 - currentLevel} cấp`}
                      </div>
                    </div>
                  </div>
                )}
