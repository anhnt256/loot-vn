...
import { Clock, Star, Trophy, GamepadIcon, ShoppingCart, Timer, Zap } from 'lucide-react';
...
export function DailyMissions({ className }: DailyMissionsProps) {
  ...
  const getTabConfig = (tab: MissionTab) => {
    switch (tab) {
      case 'ALL':
        return { label: 'Tất Cả', icon: Star, color: 'from-yellow-500 to-amber-500' };
      case 'HOURS':
      ...
      default:
        return { label: 'Khác', icon: Star, color: 'from-gray-500 to-gray-600' };
    }
  };
  ...
  const totalAvailableXP = sortedMissions.reduce((sum, m) => sum + getMissionXP(m), 0);
  
  // Calculate completion status from userCompletion data
  const completedMissions = missions.filter(m => m.userCompletion?.isDone).length;
  ...
  return (
    <Card className={`bg-gray-900/50 backdrop-blur-sm border-gray-700 ${className}`}>
      <div className="p-4">
        {/* Header - Đơn giản hóa */}
        <div className="mb-4">
          ...
          <div className="flex items-center justify-between mb-3">
            <div>
              <span className="text-white font-medium text-base">
                {activeTab === 'ALL' ? 'Tất Cả Nhiệm Vụ' : getTabConfig(activeTab).label}
              </span>
              <div className="text-sm text-gray-300 mt-1">
                <span className="text-green-400 font-semibold">{completedMissions}</span>
                <span className="text-gray-400">/{totalMissions} nhiệm vụ hoàn thành</span>
              </div>
            </div>
          ...
          <div className="bg-gradient-to-r from-amber-600/20 to-orange-600/20 border border-amber-500/30 rounded-lg p-3">
            <div className="text-center">
              ...
              <div className="text-xs">
                <span className="text-red-400 font-bold">⚠️ NHIỆM VỤ LÀM MỚI MỖI NGÀY</span>
                <span className="text-amber-300 ml-1">- Phần thưởng chưa nhận sẽ bị mất!</span>
              </div>
            </div>
          </div>
        </div>
              ...
              >
                ...
                <div className="flex items-center justify-between mb-3">
                  <div className="flex items-center gap-3">
                    <div className={`p-2 rounded-lg flex-shrink-0 ${
                      ...
                      <Icon className={`w-5 h-5 ${
                        missionStatus === 'EXPIRED' ? 'text-gray-400' : 'text-white'
                      }`} />
                    </div>
                    <div>
                      <h3 className={`font-semibold text-base ${
                        missionStatus === 'ACTIVE' ? 'text-white' 
                        : missionStatus === 'UPCOMING' ? 'text-gray-200'
                        : 'text-gray-400'
                      }`}>
                        {mission.name}
                      </h3>
                  ...
                  <span className={`text-sm font-bold ${
                    missionStatus === 'ACTIVE' ? config.textColor 
                    : missionStatus === 'UPCOMING' ? config.textColor + ' opacity-75'
                    : 'text-gray-500'
                  }`}>
                ...
                <div className="mb-4">
                  <p className={`text-sm leading-relaxed ${
                    missionStatus === 'ACTIVE' ? 'text-gray-300' 
                    : missionStatus === 'UPCOMING' ? 'text-gray-400'
                    : 'text-gray-500'
                  }`}>
                    {mission.description}
                  </p>
                ...
                <div className="flex justify-end">
                  {isCompleted ? (
                    ...
                    <Button
                      onClick={() => handleClaimMission(mission.userCompletion?.id || mission.id)}
                      ...
                      className={`bg-gradient-to-r ${config.color} text-white px-6 py-2 text-sm font-medium transition-all ${
                        claimMissionMutation.isPending ? 'opacity-75' : 'hover:opacity-90'
                      }`}
                    >
                    ...
                    >
                      {missionStatus === 'UPCOMING' ? 'Chưa Đến Giờ' : 'Không Khả Dụng'}
                    </Button>
                  )}
                </div>
              </div>
