...
export default function PenaltyManagement() {
  ...
  const fetchPenalties = async () => {
    ...
    try {
      ...
      toast.error(error.message || "Không thể tải danh sách phạt");
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSubmit = async (values: any) => {
    try {
      const response = await fetch("/api/manager/penalty", {
        method: "POST",
      ...
      const result = await response.json();
      if (result.success) {
        toast.success(result.message || "Thêm phạt thành công");
        setShowModal(false);
        form.resetFields();
        fetchPenalties();
      } else {
      ...
      toast.error(error.message || "Không thể thêm phạt");
    }
  };
    ...
    <div className="space-y-4">
      ...
      <Card>
        {loading ? (
          ...
          <Empty description="Không có dữ liệu phạt" />
        ) : (
          <Row gutter={[16, 16]}>
      ...
      >
        ...
        >
          ...
          >
            <Input placeholder="Nhập lý do phạt" />
          </Form.Item>
          ...
          <Form.Item label="Ngày phạt" name="penaltyDate">
