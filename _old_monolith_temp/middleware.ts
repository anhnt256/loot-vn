...
/** Ensure all /api/* responses are not cached (dynamic data) */
function noCacheForApi(request: NextRequest, res: NextResponse): NextResponse {
...
export async function middleware(request: NextRequest) {
  ...
  const publicPaths = [
    ...
    "/api/check-branch",
    "/api/ffood-token",
    "/api/ffood-token/test",
    "/api/ffood-shift-verify",
    "/api/fnet-shift-verify",
    ...
    "/api/momo-report",
    "/api/momo-token",
    "/api/work-shift-revenue-report/generate",
    "/api/staff",
  ...
  if (request.nextUrl.pathname.match(/^\/api\/staff\/\d+\/update-request$/)) {
    return noCacheForApi(request, response);
  }
  ...
  if (request.nextUrl.pathname.startsWith("/api/")) {
    ...
    if (!token) {
      return noCacheForApi(
        request,
        NextResponse.json({ error: "Unauthorized - No token" }, { status: 401 }),
      );
    }
